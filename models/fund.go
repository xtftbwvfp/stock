...
func NewFund(ctx context.Context, efund *eastmoney.RespFundInfo) *Fund {
	...
	stddevavg, err := utils.AvgFloat64([]float64{stddev1, stddev3, stddev5})
	if err != nil {
	...
	retavg, err := utils.AvgFloat64([]float64{ret1, ret3, ret5})
	if err != nil {
	...
	sharpavg, err := utils.AvgFloat64([]float64{sharp1, sharp3, sharp5})
	if err != nil {
...
func (f FundList) Filter(ctx context.Context, p ParamFundListFilter) FundList {
	...
	for _, fund := range f {
		...
		case p.Month3RankRatio > 0 && fund.Performance.Month3RankRatio > p.Month3RankRatio:
			// 最近3个月排名大于前百分比的排除
			continue
		case len(p.Types) > 0 && !utils.IsStrInSlice(fund.Type, p.Types):
			// 指定类型时，基金类型不在指定列表则跳过
			continue
...
func (f Fund) NetAssetsScaleHuman() string {
	return utils.YiWanString(f.NetAssetsScale)
}
