...
func StockChecker(c *gin.Context) {
	...
	keywords := utils.SplitStringFields(param.Keyword)
	if len(keywords) > 50 {
		data["Error"] = "股票数量超过限制"
		c.JSON(http.StatusOK, data)
		return
	...
	for _, stock := range stocks {
		...
		roeList := utils.ReversedFloat64Slice(
		...
		revenueList := []float64{}
		for _, r := range utils.ReversedFloat64Slice(stock.HistoricalFinaMainData.ValueList(
			c,
		...
		for _, p := range utils.ReversedFloat64Slice(stock.HistoricalFinaMainData.ValueList(
			c,
		...
		line := gin.H{
			...
			"data": [][]float64{
				roeList,
				utils.ReversedFloat64Slice(stock.HistoricalFinaMainData.ValueList(
					c,
