...
func FundIndex(c *gin.Context) {
	...
	pagi := utils.PaginateByPageNumSize(totalCount, p.PageNum, p.PageSize)
...
func FundFilter(c *gin.Context) {
	...
	// 分页
	pagi := utils.PaginateByPageNumSize(len(fundList), p.ParamFundIndex.PageNum, p.ParamFundIndex.PageSize)
	result := fundList[pagi.StartIndex:pagi.EndIndex]
	data := gin.H{
		"Env":         viper.GetString("env"),
		"Version":     version.Version,
...
func FundCheck(c *gin.Context) {
	...
	codes := utils.SplitStringFields(p.Code)
...
func FundSimilarity(c *gin.Context) {
	...
	codeList := utils.SplitStringFields(p.Codes)
...
func FundManagers(c *gin.Context) {
	...
	// 分页
	pagi := utils.PaginateByPageNumSize(len(managers), p.PageNum, p.PageSize)
