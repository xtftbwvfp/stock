...
main() {
    ...
    # git clone https://github.com/ykstudy/pink-lady.git ${gopath}/src/${projname}
    rm -
    if !(curl https://codeload.github.com/ykstudy/pink-lady/zip/master -o /tmp/pink-lady.zip && unzip /tmp/pink-lady.zip -d /tmp)
    then
    ...
    if [ `uname` = 'Darwin' ]; then
        sed -i '' -e "s|github.com/ykstudy/pink-lady|${projname}|g" `grep "pink-lady" --include "swagger.*" --include ".travis.yml" --include "*.go" --include "go.*" -rl .`
    else
        sed -i "s|github.com/ykstudy/pink-lady|${projname}|g" `grep "pink-lady" --include "swagger.*" --include ".travis.yml" --include "*.go" --include "go.*" -rl .`
    fi
